...
doc = """The Dutch version of Public Goods Game with endogeneous threshold
"""


class Constants(BaseConstants):
    name_in_url = 'PGGendoNL'
    players_per_group = 4
    num_rounds = 12
...
class Group(BaseGroup):
    ...
    def set_distribution_rule(self):
        if self.round_number in Constants.phase1:
            self.distribution_rule = 'Gelijk deel van de bonus'
    ...
    def set_threshold(self):
        self.threshold = min([p.proposal for p in self.get_players()
                              if p.proposal is not None])

        """"The lowest proposed threshold by 
        a player will become the round's threshold level."""
    ...
    def set_payoffs(self):
        ...
        if self.distribution_rule == 'Gelijke verdiensten':
            if self.total_contribution < self.threshold:
                for p in self.get_players():
                    p.payoff_r = p.value
            else:
...
class Player(BasePlayer):
    ...
    proposal = models.CurrencyField(
        min=0, max=Constants.endowment*Constants.players_per_group,
        doc="""The proposed threshold level for the current round by the player.""",
     )
    ...
    prule = models.PositiveIntegerField(
        choices=[
            [0, 'Gelijk deel van de bonus'],
            [1, 'Gelijke verdiensten']
        ],
